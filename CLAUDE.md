...
- `NAV_WAIT_UNTIL`/`NAV_TIMEOUT`/`NAV_FAIL_FAST_4XX`, `SCANNER_ID`, `PRESCAN_SCROLL`, `TESTARO_RULE_RETRIES` — deployment defaults that job properties (`navigation`, `scannerId`, `scroll`, `browserChannel`; `retries` on test acts) override (`procs/config.js`). (`QUALWEB_STEALTH`/`QUALWEB_ADBLOCK` and a `qualWeb` act's `stealth`/`adBlock` no longer have any effect: qualWeb runs in the page under test.)
