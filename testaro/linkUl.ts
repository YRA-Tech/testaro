...
/*
  ...
  Compiled to linkUl.js by tsc (issue #73); edit this file, not the emitted one.
*/
...
export const reporter = async (page: Page, report: Report, _: unknown, withItems: boolean) => {
  // The candidate selector yields rendered HTML link elements.
  const getBadWhat = (element: HTMLElement): BadWhat => {
    let {display, textDecorationLine}: {display: string | null; textDecorationLine: string} = window.getComputedStyle(element);
    ...
    if (! textDecorationLine.includes('underline')) {
      ...
      const compact = (string: string | null | undefined) => string?.replace(/\s+/g, ' ')?.trim();
