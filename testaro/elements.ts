...
// Data on one sibling node of an element.
interface SibInfo {
  type: number;
  tagName?: string;
...
interface ElementDatum {
  ...
  siblings?: {before: SibInfo[]; after: SibInfo[]};
}
...
export const reporter = async (
  ...
  try {
    ...
    data = await locator.evaluateAll((elements, detailLevel) => {
      ...
      const getSibInfo = (node: Node, nodeType: number, text: string | null) => {
        const sibInfo: SibInfo = {
          type: nodeType
        };
        // If the sibling node is an element:
        if (nodeType === 1) {
          sibInfo.tagName = (node as Element).tagName;
          sibInfo.attributes = [];
          Array.from((node as Element).attributes).forEach(attribute => {
      ...
      const data: {total: number; items?: ElementDatum[]} = {
      ...
      // Otherwise, i.e. if itemization is required:
        ...
        elements.forEach(element => {
          ...
          const {labels, textContent} = element as HTMLInputElement;
    ...
    console.log(`ERROR performing test (${(error as Error).message})`);
    data = {
      prevented: true,
