...
// Data on one attribute of an element.
interface AttributeDatum {
  name: string;
  value: string;
}
// Data on an element, possibly with data on its descendants.
...
/*
  ...
  Compiled to textNodes.js by tsc (issue #73); edit this file, not the emitted one.
*/
...
export const reporter = async (
  page: Page, _0: unknown, _1: unknown, _2: unknown, detailLevel: number, text = ''
) => {
  ...
  try {
    data = await page.evaluate(args => {
      ...
      // Compacts a string.
      const compact = (string: string) => string.replace(/\s+/g, ' ').trim();
      // Compacts and lower-cases a string.
      const toStandardString = (string: string) => compact(string).toLowerCase();
      /*
        Gets data (tagName, text if specified, attributes, refLabels, labels, and children)
        on an element.
      */
      const getElementData = (element: Element, withText: boolean): ElementData => {
        // Initialize the data.
      ...
      const data: {
        nodeCount: number;
        items?: {text: string; ancestors?: ElementData[]}[];
      } = {nodeCount: 0};
      let more = true;
      while(more) {
        if (walker.nextNode()) {
      ...
      if (detailLevel > 0) {
        ...
        matchNodes.forEach(node => {
          ...
          const itemData: {text: string; ancestors?: ElementData[]} = {
            text: compact(node.nodeValue!)
          };
      ...
      return data;
    }, [detailLevel, text] as const);
  }
  catch(error) {
    console.log(`ERROR performing test (${(error as Error).message.replace(/\n.+/s, '')})`);
    data = {
      prevented: true,
      error: 'Test failed'
    };
